...
const AgentCompanyDashboard = () => {
    ...
    const [events, setEvents] = useState([]); // Lifted state for ActivityFeed persistence
    ...
    useEffect(() => {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const mappedTasks = data.tasks.map(t => ({
                        ...
                        assignedTo: getAgentIdFromDbId(t.assignedToId),
        ...
        socket.on('agent:task:start', (data) => {
            ...
            setTasks(prev => prev.map(t => t.id === data.taskId ? { ...t, status: 'In Progress', progress: 10 } : t));
        });

        socket.on('agent:task:complete', (data) => {
            console.log('Task complete:', data);
            setTasks(prev => prev.map(t => t.id === data.taskId ? { ...t, status: 'Complete', progress: 100 } : t));
        });
        ...
        socket.on('agent:status', (data) => handleEvent('status', data, `Status changed to ${data.status}`));
    ...
    // Helpers
    const getAgentIdFromDbId = (dbId) => {
        // Simple mapping or return as is if IDs match. Our hardcoded agents have short IDs (backend, design), DB might use UUIDs or same.
        // If database uses UUIDs, we need to find the agent with that DB ID. But currently agents hardcoded above have simple IDs.
        // Assuming mapped agents from API update the 'id' field in state.
        return dbId || 'unassigned';
    };
            ...
            <div style={{ flex: 1, padding: '48px', overflowY: 'auto' }}>
                ...
                {activeView === 'dashboard' && (
                    <div>
                        ...
                        {/* Agents Grid - Padding right to account for ActivityFeed overlay */}
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px', paddingRight: '380px' }}>
                            {agents.map(agent => (
                                <div
                                    key={agent.id}
                ...
                {/* Persistent Live Activity Feed - Visible on Dashboard, or potentially globally via a toggle. For now, putting it in Dashboard view right column */}
                {activeView === 'dashboard' && (
                    <div style={{
                        position: 'fixed',
                        right: '32px',
